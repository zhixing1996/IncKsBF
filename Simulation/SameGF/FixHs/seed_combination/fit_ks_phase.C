...
int main(int argc, char *argv[]){
	...
	TString apple="/besfs/groups/tauqcd/jingmq/IncKSBF/SameGF/FixHs/RootFile/fit_ks_phase";
	...
	ofstream paras;
	TString orange="/besfs/groups/tauqcd/jingmq/IncKSBF/SameGF/FixHs/Parameter/parameter";
	for (int i=1; i<argc;i++){
		orange+="_";
		orange+=argv[i];
	}
	...
	stringstream num;
	num<<argv[argc-1];
	TString dog="../../C/Combination"+num.str()+"/C.txt";
	ifstream C_C(dog);
	double CC[50];
	for (int i=0;i<50;i++){
		C_C>>CC[i];
		stringstream C1;
		C1 << CC[i];
		TString grape="../../C/Combination"+num.str()+"/"+C1.str()+"/cross_section.txt";
	...
	TString banana="/besfs/groups/tauqcd/jingmq/IncKSBF/SameGF/FixHs/CrossSection/cross_section";
	for (int i=1; i<argc;i++){
		banana+="_";
		banana+=argv[i];
	}
	...
	for (int i=0;i<50;i++){
		stringstream C2;
		C2 << CC[i];
		TString monkey="../../C/Combination"+num.str()+"/"+C2.str()+"/branch_ratio.txt";
		ifstream BF(monkey);
		BF>>bf;
		sum_bf+=bf;
	}
